...
public class UpdateHandler : IUpdateHandler
{
    ...
    private async Task ProcessMessageAsync(ITelegramBotClient botClient, Message message, BotDbContext dbContext, CancellationToken cancellationToken)
    {
        ...
        if (message.Text.StartsWith("/start"))
        {
            var buttons = new List<InlineKeyboardButton[]> { new[] { InlineKeyboardButton.WithWebApp("🌌 Открыть KoFFPanel", new WebAppInfo { Url = "https://5080ea0e829c10.lhr.life" }) } }; // ВСТАВЬ СВОЮ ССЫЛКУ
